...
def is_point_in_bbox(point, bbox):
    """
    Check if a point is inside a bounding box (for GUI tasks)
...
def get_box_center(box):
    """
    Calculate the center point of a bounding box (for GUI tasks)
...
def extract_gui_predictions(extracted_predictions):
    """
    Extract all point and box coordinates from the extracted_predictions dict (for GUI tasks)
...
def has_wrong_rejection(pred_boxes_dict):
    """
    Check whether predictions contain an incorrect rejection (predicted None on a non-hallucination task).

    Args:
        pred_boxes_dict: Prediction dict, e.g. {"category": [boxes_or_None]}

    Returns:
        bool: Whether an incorrect rejection is present
    """
...
def calculate_gui_metrics(gt_box, extracted_predictions):
    """
    Compute correctness for GUI localization (whether any prediction hits the ground-truth box).
    ...
    # If there are no predicted points or boxes, treat as failure
    if not predicted_points and not predicted_boxes:
        return False

    # If GT is empty or malformed, treat as failure
    ...
    # Check if the center of any predicted box lies inside the GT box
...
def calculate_pointing_metrics(gt_masks_dict, pred_points_dict):
    ...
    1. Mask format: dict with "counts" and "size" keys
    2. Bbox format: list of 4 values [x1, y1, x2, y2]
    """
    # Count total GT masks/boxes and predictions
    total_gt_count = sum(len(items) for items in gt_masks_dict.values())
    ...
    # Flatten all GT items (masks or bboxes) and points for matching
    all_gt_items = []  # List of (item, item_type, category) where item_type is 'mask' or 'bbox'
    all_pred_points = []
    all_gt_categories = []
    all_pred_categories = []

    # Detect GT format by checking items across all categories
    use_bbox_format = None  # None means not determined yet
    for category in gt_masks_dict:
        category_items = gt_masks_dict[category]
        if not isinstance(category_items, list) or len(category_items) == 0:
            continue
        
        # Check items to determine format
        for item in category_items:
            if isinstance(item, list) and len(item) == 4:
                # Bbox format: [x1, y1, x2, y2]
                use_bbox_format = True
                break
            elif isinstance(item, dict) and "counts" in item and "size" in item:
                # Mask format
                use_bbox_format = False
                break
        
        if use_bbox_format is not None:
            break
    
    # Default to bbox format if format cannot be determined
    if use_bbox_format is None:
        use_bbox_format = True

    # Process GT items based on detected format
    for category in gt_masks_dict:
        category_items = gt_masks_dict[category]
        # Ensure category_items is a list
        ...
        for item in category_items:
            if use_bbox_format:
                # Bbox format: [x1, y1, x2, y2]
                if isinstance(item, list) and len(item) == 4:
                    all_gt_items.append((item, 'bbox', category))
                    all_gt_categories.append(category)
            else:
                # Mask format: dict with "counts" and "size"
                if isinstance(item, dict) and "counts" in item and "size" in item:
                    all_gt_items.append((item, 'mask', category))
                    all_gt_categories.append(category)
    ...
    # Pre-decode all masks if using mask format
    decoded_masks = []
    if not use_bbox_format:
        for gt_item, item_type, category in all_gt_items:
            if item_type == 'mask':
                mask = decode_rle(gt_item["counts"], gt_item["size"])
    ...
    # For each GT item, find the best matching prediction
    for i, (gt_item_info, gt_category) in enumerate(
        zip(all_gt_items, all_gt_categories)
    ):
        gt_item, item_type, _ = gt_item_info
        ...
        for j, (pred_point, pred_category) in enumerate(
            ...
            # Check if point matches based on GT format
            is_match = False
            if item_type == 'bbox':
                # Check if point is in bbox
                is_match = is_point_in_bbox(pred_point, gt_item)
            elif item_type == 'mask':
                # Check if point is in mask
                if i < len(decoded_masks) and decoded_masks[i] is not None:
                    is_match = is_point_in_mask(pred_point, decoded_masks[i])

            if is_match:
                # For pointing task, we consider it a match if the point is in the bbox/mask
                # and it's the first match for this GT item
                if best_match == -1:
                    best_match = j
                    best_score = 1.0
...
class UniversalMetricsCalculator:
    ...
    def calculate_metrics_for_sample(self, sample, iou_threshold=0.5):
        ...
        if task_name == "gui":
            ...
            # Ensure gt_boxes is a dict before converting
            if not isinstance(gt_boxes, dict):
                print(f"Warning: gt_boxes is not a dict for task '{task_name}': {type(gt_boxes)}, value: {gt_boxes}")
                # Try to handle different formats
                if isinstance(gt_boxes, list):
                    # If it's a list, create a default dict structure
                    gt_boxes = {"default": gt_boxes}
                else:
                    gt_boxes = {}
            else:
                gt_boxes = {k.lower(): v for k, v in gt_boxes.items()}
            
            # Ensure pred_boxes is a dict before converting
            if isinstance(pred_boxes, dict):
                pred_boxes = {k.lower(): v for k, v in pred_boxes.items()}
            else:
                print(f"Warning: pred_boxes is not a dict for task '{task_name}': {type(pred_boxes)}, value: {pred_boxes}")
                pred_boxes = {}
        ...
        if task_name == "referring_object_detection":
            ...
            # Ensure gt_boxes is a dict (it should be after the lowercase conversion above)
            if not isinstance(gt_boxes, dict):
                print(f"Warning: gt_boxes is not a dict for pointing task: {type(gt_boxes)}, value: {gt_boxes}")
                # Try to convert if it's a list or other format
                if isinstance(gt_boxes, list):
                    # If gt_boxes is a list, create a default dict structure
                    gt_boxes = {"default": gt_boxes}
                else:
                    gt_boxes = {}
            
            # Ensure pred_boxes is a dict
            if not isinstance(pred_boxes, dict):
                print(f"Warning: pred_boxes is not a dict for pointing task: {type(pred_boxes)}, value: {pred_boxes}")
                pred_boxes = {}
