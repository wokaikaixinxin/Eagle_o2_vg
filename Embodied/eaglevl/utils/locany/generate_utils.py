...
def get_token_ids_from_config(config) -> Dict[str, int]:
    ...
    token_ids['box_coord_dim'] = getattr(config, 'box_coord_dim', 4)
...
def is_valid_box_frame(
    ...
    # A complete box frame is ``<box><c1>...<cN></box>``.  Do not assume
    # N=4 here: rotated boxes use N=5 (cx, cy, w, h, angle), while the
    # legacy oriented-polygon format uses N=8.
    box_coord_dim = int(token_ids.get('box_coord_dim', 4))
    box_end_position = box_coord_dim + 1
    if probs.size(0) <= box_end_position:
        return 'illegal_box'
    ...
    end_score = probs[box_end_position, end_target_ids].sum()
...
def decode_bbox_avg(
    ...
    box_coord_dim = int(token_ids.get('box_coord_dim', 4))
    if probs.size(0) < box_coord_dim + 2:
        return None

    # Extract coordinate probabilities and compute Top-K for all coordinate positions.
    pos_probs, pos_ids = torch.topk(probs[1:1 + box_coord_dim], k=keep_k, dim=-1)
    ...
    if generation_mode == 'hybrid':
        ...
        # Spatial coordinates use their ordinary top-k token span. For a
        # cxcywha box, angle is periodic over 180 degrees, so it needs a
        # circular span rather than max-min (e.g. angle bins 2 and 998 are
        # close, not far apart).
        is_abnormal = torch.zeros_like(first_valid_probs, dtype=torch.bool)
        spatial_dim = 4 if box_coord_dim == 5 else box_coord_dim
        spatial_range = valid_max[:spatial_dim] - valid_min[:spatial_dim]
        is_abnormal[:spatial_dim] = (
            (first_valid_probs[:spatial_dim] < 0.9)
            & (valid_counts[:spatial_dim] > 1)
            & (spatial_range > 60)
        )

        if box_coord_dim == 5:
            angle_candidates = pos_ids[4][mask[4]] - coord_start_token_id
            sorted_angles = torch.sort(angle_candidates).values
            angle_gaps = torch.cat((
                sorted_angles[1:] - sorted_angles[:-1],
                sorted_angles[:1] + 1000 - sorted_angles[-1:],
            ))
            circular_angle_range = 1000 - angle_gaps.max()
            is_abnormal[4] = (
                (first_valid_probs[4] < 0.9)
                & (valid_counts[4] > 1)
                & (circular_angle_range > 60)
            )
...
def handle_pattern(x0, token_ids: Dict[str, int], generation_mode: str = 'hybrid'):
    ...
    if x0[0] == null_token_id:
        ...
        coord_ix = 1
        for coord in x0[1:1 + box_coord_dim]:
            if coord_start_token_id <= coord <= coord_end_token_id:
                coord_ix += 1
            else:
                break

        # Box with configurable coordinate count:
        # <box><c1>...<cN></box>, where N defaults to 4 and is 8 for oriented boxes.
        if (
            coord_ix == box_coord_dim + 1
            and len(x0) > box_coord_dim + 1
            and x0[box_coord_dim + 1] == box_end_token_id
        ):
            return {
                "type": "coord_box",
                "tokens": x0,
