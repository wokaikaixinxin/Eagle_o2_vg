...
def update_causal_mask_with_pad_non_visible_2d(
    ...
    Args:
        ...
        text_mask_token_id: ID representing masked tokens (required, no default)
...
def create_block_diff_mask_by_pe_4d(
    ...
    Returns:
        tuple[torch.Tensor, torch.Tensor]:
            - A float mask of shape [batch_size, 1, seq_len, seq_len] with `-inf` for masked positions (non visible).
...
def find_pred_pos_from_input_ids(
    ...
    Args:
        ...
        text_mask_token_id (int): Token ID representing masked positions (required).
