...
try:
    ...
    print("deep_gemm not found, please install it")
import re
import types
...
def deep_matmul(a, b, out=None):
    ...
    m, k = a.shape
    n, k2 = b.shape
    assert k == k2 and k % 128 == 0, f"k: {k}, k2: {k2}, a.shape: {a.shape}, b.shape: {b.shape}"
    a = pad_8(a)
...
# Used by TransformerEngine, can be ignored
def deep_matmul_pad256(a, b, out=None):
    # Note: a is [m,k], b is [n,k], contiguity not required
    assert a.dim() == 2 and a.dtype == torch.bfloat16
    assert b.dim() == 2 and b.dtype == torch.bfloat16
    m, k = a.shape
    n, k2 = b.shape
...
def apply_fp8(module, pattern=['down_proj'], prefix=None):
    '''
    ...
    for name, child in module.named_children():
        ...
        if 'vision' in full_name:
            continue
        
        if 'self_attn' in full_name:
            continue
        
        if full_name.startswith('language_model.model.layers.'):
            layer_idx = full_name.replace('language_model.model.layers.', '').split('.')[0]
            if int(layer_idx) % 2 == 0:
                continue
        
        if isinstance(child, torch.nn.Linear):
            for p in pattern:
                if re.search(p, name):
                    print('apply fp8 for: ', full_name, child.weight.size())
                    child.forward = types.MethodType(fp8_forward, child)
                    break     
        apply_fp8(child, pattern, full_name)
