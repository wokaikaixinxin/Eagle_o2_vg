...
class LocateAnythingWorker:
    ...
    def _predict_standard(
        ...
        n_future_tokens = getattr(
            getattr(self.model.config, "text_config", None),
            "block_size",
            getattr(self.model.config, "block_size", 6),
        )
        box_coord_dim = getattr(self.model.config, "box_coord_dim", 4)
        # MTP predicts <box>, N coordinates, and </box> in one frame.
        # This is seven tokens for the cxcywha protocol.
        n_future_tokens = max(n_future_tokens, int(box_coord_dim) + 2)
    ...
    def detect_rotated(self, image: Image.Image, categories: Union[list[str], str], **kwargs) -> dict:
        """Rotated-object detection using ``(cx, cy, w, h, angle)`` boxes.

        The five values are normalized integer bins: ``cx, w`` use image
        width, ``cy, h`` use image height, and ``angle`` maps the canonical
        long-edge angle in degrees from ``[-90, 90)`` to ``[0, 1000)``. A model must be fine-tuned
        on this protocol with ``box_coord_dim=5`` to produce meaningful output.
        """
        cats = categories if isinstance(categories, str) else "</c>".join(categories)
        prompt = (
            "Locate all the rotated bounding boxes that match the following "
            f"description: {cats}. Return each box as center-x, center-y, width, "
            "height, and canonical long-edge angle in [-90, 90) degrees."
        )
        return self.predict(image, prompt, **kwargs)
    ...
    @staticmethod
    def parse_rotated_boxes(answer: str, image_width: int, image_height: int) -> list[dict]:
        """Parse five-coordinate rotated boxes into pixels and degrees.

        Expected serialization is ``<ref>label</ref><box><cx><cy><w><h><a></box>``.
        The angle bin represents the canonical long-edge angle in degrees:
        ``a / 1000 * 180 - 90``.
        """
        boxes = []
        pattern = (
        ...
        for m in re.finditer(pattern, answer):
            label = m.group(1)
            cx, cy, width, height, angle = [int(g) for g in m.groups()[1:]]
            if width <= 0 or height <= 0:
                continue
            boxes.append({
                "label": label,
                "cx": cx / 1000 * image_width,
                "cy": cy / 1000 * image_height,
                "w": width / 1000 * image_width,
                "h": height / 1000 * image_height,
                "angle": angle / 1000 * 180.0 - 90.0,
            })
        return boxes
