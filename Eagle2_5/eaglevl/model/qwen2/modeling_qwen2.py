...
if is_flash_attn_2_available():
    try:
        ...
        # print("use_flash_attn3:", use_flash_attn3, flush=True)
    except ImportError:
...
class Qwen2RotaryEmbedding(nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        # print(self.cos_cached.shape, self.cos_cached.dtype, self.sin_cached.shape, self.sin_cached.dtype, flush=True)
    ...
    def forward(self, x, seq_len=None):
        ...
        return self._online_get_cos_sin(seq_len, x.device)
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    ...
    q_embed = q_embed.to(org_type)
    k_embed = k_embed.to(org_type)
...
class Qwen2FlashAttention2_packing(Qwen2Attention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.ring_attn_fn = zigzag_ring_flash_attn_varlen_func
    ...
    def forward(
        ...
        **kwargs,
        ...
        if get_pg_manager() is not None and get_pg_manager().sequence_parallel_world_size > 1:
            # if using SP, use length of the whole seq
            rotary_seq_len = kv_seq_len * get_pg_manager().sequence_parallel_world_size
        else:
            rotary_seq_len = max(kv_seq_len, position_ids[:, -1].max().item()) + 1
        cos, sin = self.rotary_emb(value_states, seq_len=rotary_seq_len)
        # print(kv_seq_len, position_ids[:, -1].max().item(), rotary_seq_len, position_ids.shape[1], cos.shape, sin.shape)
        ...
        #     if get_pg_manager() is not None and get_pg_manager().sequence_parallel_world_size > 1:
        #         packing_position_ids = ring_split_for_sequence_parallel(packing_position_ids, channel_dim=1, sp_group=get_pg_manager().sequence_parallel_group, 
        #             ulysses_group=get_pg_manager().ulysses_sequence_parallel_group, ring_group=get_pg_manager().ring_sequence_parallel_group, sub_sample_lengths=sub_sample_lengths) 
    ...
    def _flash_attention_forward(
        ...
        # print("test enable_sequence_parallel", flush=True)
        enable_sequence_parallel = (
            dist.is_initialized() and get_pg_manager() is not None and get_pg_manager().sequence_parallel_world_size > 1
        )
        # print("test enable_sequence_parallel done")
        # print("pre_process_for_sequence_parallel_attn", query_states.shape, key_states.shape, value_states.shape)
        if enable_sequence_parallel:
            ...
            # print(f"query_states[rank {dist.get_rank()}, after SP]:", query_states.shape)

        # print("pre_process_for_sequence_parallel_attn done", query_states.shape, key_states.shape, value_states.shape)
        ...
        if attention_mask is not None:
            ...
            # print("unpad_input_packing done", query_states.shape, key_states.shape, value_states.shape)
            ...
            max_seqlen_in_batch_q, max_seqlen_in_batch_k = max_seq_lens
            # print(f"""
            # SP_rank: {get_pg_manager().sequence_parallel_rank}
            # ulysses_SP_rank: {get_pg_manager().ulysses_sequence_parallel_rank}
            # ring_SP_rank: {get_pg_manager().ring_sequence_parallel_rank}
            # DP_rank: {get_pg_manager().data_parallel_rank}
            # cu_seqlens_q: {cu_seqlens_q}
            # cu_seqlens_k: {cu_seqlens_k}
            # max_seqlen_in_batch_q: {max_seqlen_in_batch_q}
            # max_seqlen_in_batch_k: {max_seqlen_in_batch_k}
            # query_states: {query_states.mean()}
            # key_states: {key_states.mean()}
            # value_states: {value_states.mean()}
            # """, flush=True)
            # dist.barrier()
            # exit()
            if not use_sliding_windows:
                if get_pg_manager() is not None and get_pg_manager().ring_sequence_parallel_size > 1:
                    assert torch.equal(cu_seqlens_q, cu_seqlens_k), "cu_seqlens_q and cu_seqlens_k must be the same"
                    warnings.warn("Attention Type: Using ring attention")
                    ...
                    warnings.warn("Attention Type: Using flash attention")
                    if use_flash_attn3:
    ...
    def _unpad_input_packing(self, query_layer, key_layer, value_layer, attention_mask, query_length, sub_sample_lengths):
        ...
        # print(f"key_layer1[{dist.get_rank()}]", key_layer.shape)

        key_layer = index_first_axis(key_layer.reshape(batch_size * kv_seq_len, num_heads, head_dim), indices_k)
        value_layer = index_first_axis(value_layer.reshape(batch_size * kv_seq_len, num_heads, head_dim), indices_k)
        # print(f"key_layer2[{dist.get_rank()}]", key_layer.shape)
        # dist.barrier()
        # exit()
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print("LN hidden_states:", hidden_states.shape)
